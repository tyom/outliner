...
module.exports = {
  ...
  augment: function(source) {
    var elements = [];
    var parser = this;

    source.each(function(i, item) {
      walker.walk(item, function(depth) {
        if(!this.name || this.type === "script") {
          return;
        }

        this.depth = depth;
        this.components = parser.getComponents(this);
        this.parts = parser.getParts(this);
        this.modifiers = parser.getModifiers(this);
        this.states = parser.getStates(this);
        this.id = this.attribs.id;
